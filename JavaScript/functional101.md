...
```JS
// The global variable
// this global variable does not get changed with these functions below
var bookList = ["The Hound of the Baskervilles", "On The Electrodynamics of Moving Bodies", "Philosophiæ Naturalis Principia Mathematica", "Disquisitiones Arithmeticae"];

function add (arr, bookName) {
  // this copies the array passed to it, and appends the final item
  return [...arr, bookName];
}

function remove (arr, bookName) {
  // this returns the passed in array, but removes any items that match what was passed
...
// using more complex array destructuring
const ratings_map_2 = watchList.map(({ Title: title, imdbRating: rating }) => ({title, rating}));

var ratings_for_loop = [];
for(var i=0; i < watchList.length; i++){
  ratings_for_loop.push({title: watchList[i]["Title"],  rating: watchList[i]["imdbRating"]});
}
...
- Building a map method from scratch
  ...
  - Note the use of `this` which is only because we're using the "class" Array
...
console.log(new_s) // returns [ 46, 130, 196, 10 ]
...
console.log(moviesAndRatings_8_up);
...
Array.prototype.myFilter_forLoop = function(callback) {
  var newArray = [];
  for (let i = 0; i < this.length; i++) {
...
// using the first method to filter
var new_s = s.myFilter_forEach(function(item){
  return item % 2 === 1;
});
